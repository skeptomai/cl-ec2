;; -*- Mode: Lisp; External-format: Latin-1; rcs-header: "$Header: /hope/lwhope1-cam/hope.0/compound/46/LISPconfig/RCS/a-dot-lispworks.lisp,v 1.32.1.2 2006/04/18 17:00:49 davef Exp $" -*-
...
(setf *packages-for-warn-on-redefinition*
      '("AMD64" "ATSUI" "CAPI" "CAPI-COCOA-LIBRARY" "CAPI-INTERNALS"
        ...
        "FOREIGN-PARSER" "GENERATOR" "GRAPHICS-PORTS"
        "HARLEQUIN-COMMON-LISP" "HARP" "HQN-WEB" "INS"
        "INTERFACE-BUILDER" "KEYWORD" "KW" "KW-TOOLS" "LISPWORKS"
        "LISPWORKS-TOOLS" "LOOP" "LOW64" "LW-XP" "MAIL" "MATCH" "MP"
        "OBJC" "ODBC-COMMON" "OMG.ORG/OPERATION"
        "OMG.ORG/PORTABLESERVER" "OMG.ORG/ROOT" "PARSERGEN" "PC386"
...
(setf (editor:variable-value "Check Mismatched Parens" :mode "Lisp")
       :warn)


;; This makes the editor allow OPEN to determine if a file is LF or
;; CRLF terminated.  The value (:latin-1 :eol-style :lf) would make
;; the editor read every file as Latin-1 LF-terminated.

(setf (editor:variable-value "Input Format Default") nil)


;; This makes the editor write Latin-1 LF-terminated files
...
;; When set to non-NIL, tells defsystem to print any compilation
;; errors as warnings and continue.

(setf scm:*continue-from-compile-error* nil)


;; When set to non-NIL, tells defsystem to print any load errors as
...
;; Load asdf, and then use asdf to load asdf-install, et. al
(load 
 (merge-pathnames 
...
(mapcar #'(lambda (x) (asdf:oos 'asdf:load-op x)) *my-systems*)
...
(format t "~{~a~^,~}~%" system:*line-arguments-list*)
(let ((script (and (sixth system:*line-arguments-list*)
                   (probe-file (sixth system:*line-arguments-list*)))))
  (when script
    ;; Handle shebang-line
